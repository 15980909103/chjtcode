...
var $api = (function() {
	...
	const createHouseList = ( data, type )=>{
		...
		for(let i in list){
			...
			if( item.have_cover ){
				cover = item.have_cover;
			} else {
				cover = ( item.detail_cover && item.logo ) ? 1 : 0;
			}
			
			if( Object.keys(item).indexOf('area_str') != -1 &&  Object.keys(item).indexOf('business_area_str') != -1 ){
				site = item.area_str + ' ' + item.business_area_str;
			} else {
				site = item.area + ' ' + item.business;
			}
			...
			const obj = {
				...
				info: {
					...
					lab: [],
					lng: item.lnglat ? item.lnglat.lng : '',
					lat: item.lnglat ? item.lnglat.lat : '',
					address: item.business + item.street + item.address
				},
				cover: cover,
				img: [item.list_cover]
			};
			...
			(house_purpose_arr.length > 0)&&house_purpose_arr.map( tip=>{
			...
			item.feature_tag&&item.feature_tag.map( tip=>{
			...
			item.selling_point&&item.selling_point.map( point=>{
				let objP = {
					name: point.title
				}
