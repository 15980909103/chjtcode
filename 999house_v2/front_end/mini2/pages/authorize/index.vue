		...
		onLoad() {
			...
			uni.getSetting({
				success: function (res) {
					...
					if (res.authSetting['scope.userInfo']&&app.globalData.token&&that.hasGetMobile==1) {// 已经用户信息授权，//存在请求token，同时启用手机授权
		...
		methods: {
			...
			bindGetUserInfo: function (e) {
				...
				if(e.detail.encryptedData) {
					that.$api.wxMiniAuthLogin(app, {
						encryptedData: e.detail.encryptedData,
						iv: e.detail.iv,
					}).then(function(res){
						that.$data.isclick = false;				
						console.log(res,'authpage')
						if(!res){
							that.$toast('授权失败:'+res+ '请重试')
							return;
						}
			...
			doResponse(res){
				...
				if(pages.length>1){
					...
					if(!app.globalData.restopen){
						...
						that.goPage(
							app.globalData.restopen, //回跳最开始要打开的页面
							'',
							'reLaunch'
						)
					}
