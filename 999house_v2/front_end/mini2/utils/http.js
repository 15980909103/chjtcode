...
const domain = 'http://act.999house.com/';
...
const $http = (function() {
	...
	const publicFun = (e) =>{
		...
		let allTag_update = $api.localStore.localGet('u-tag-update');
	...
	const getMapCity = ()=> {
		return new Promise((resolve,reject)=>{
			...
			const cityList = $api.localStore.localGet('city-list');
			if(!map){
				resolve('');
	...
	const getCurrentCity = (val)=> {
		...
		const that = this;
		const cityList = $api.localStore.localGet('city-list', 3600*2);
		...
		let res = $api.localStore.localGet(key);
...
const loginMixin = {
	...
	created (){
		...
		//微信h5微信没登录进行登录
		console.log('http',$http.getLocal('token'))
		if(!$http.getLocal('token') ){
			...
			$http.wxLogin(0,(e)=>{
				...
				if( e == 1 ){
					const url = this.$api.localStore.localGet('pre-page');
