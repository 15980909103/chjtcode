    ...
    <div class="mapmange_content">
      ...
      <el-table :data="tableData" style="width: 100%">
        ...
        <el-table-column  label="icon图标"  align="center">
          <template slot-scope="scope">  
            <img style="width:50px; height:50px;" :src="getRealImgUrl(scope.row.icon)"/>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作"  align="center">
          <template slot-scope="scope">
            <el-button type="primary" size="mini" @click="doEdit({ id:scope.row.id, name:scope.row.name,type:scope.row.type, fordo:scope.row.fordo, icon:scope.row.icon })">编辑</el-button>
      ...
      >
        <el-form style="padding-right:50px;" :model="form" ref="form" :rules="rules">
          ...
          <el-form-item label="上传图片" :label-width="formLabelWidth" prop="icon">
  ...
  data() {
    ...
    return {
      ...
      form: {
        ...
        type: 2,
        fordo: '',
        icon: '/static/images/defultImg.png'
      },
  ...
  methods: {
    ...
    doEdit(e={}){
      ...
      if(Object.keys(e).length>0){
        ...
        that.default_src= that.getRealImgUrl(that.form.icon)
      }
    },
    doEditSubmit(){
      let that=this
