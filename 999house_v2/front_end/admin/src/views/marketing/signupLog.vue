  ...
  <div class="_container">
    <el-form :inline="true" :model="searchData" class="form-serch">
      ...
      <el-form-item>
            <eport-excel :setArr='{
                post:{
                    url:"signup/getLogList",
                    total: "1000",
                    pagesize: "1000",
                    data:exdata
                },
                filename: "报名记录",
                tHeader:["ID","楼盘","昵称","手机号码","创建时间"],
                filterVal:["id","forname","nickname","phone","create_time"],
                ajaxSuccess: exportexcelSuccess
            }'></eport-excel>
        </el-form-item>
    ...
    <el-table :data="tableData" style="width:100%">
      ...
      <el-table-column prop="forname" label="楼盘" align="center"></el-table-column>
...
import Eportexcel from '@/components/common/exportexcel.vue'
...
export default {
  ...
  created: function(){
    let that = this
    that.setDataArr({
      signup_id: that.$urlData.signup_id,
      region_no: that.$urlData.region_no,
      type: that.$urlData.type,
      searchData:{
        signup_id: that.$urlData.signup_id,
        region_no: that.$urlData.region_no,
        type:that.$urlData.type
    ...
    if(!that.signup_id||!that.region_no){
      this.$alert('缺失参数', '提示',{
        confirmButtonText: '确定',
        callback: action => {
          util.openPage({url:'/marketing/signup'});
        }
      });
      return;
    }
  ...
  methods: {
      exportexcelSuccess(res){
       
        return res.data.list;
      },

    getList(searchdata={}){
     this.exdata = this.deepClone(searchdata)
     this.exdata.do_export = 1
      var that = this
