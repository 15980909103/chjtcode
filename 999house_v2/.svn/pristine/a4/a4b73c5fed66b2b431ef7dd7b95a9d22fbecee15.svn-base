 ...
 */
class MerchantMenuValidate extends BaseValidate
{
    ...
    protected function checkParentId($value,$rule,$data=[])
    {
        ...
        $find = $this->db->name('merchant_mymenu')->where("id", $value)->value('parent_id');

        if ($find) {
            $find2 = $this->db->name('merchant_mymenu')->where("id", $find)->value('parent_id');
            if ($find2) {
                $find3 = $this->db->name('merchant_mymenu')->where("id", $find2)->value('parent_id');
                if ($find3) {
                    return '超过了4级';
                }
            }
