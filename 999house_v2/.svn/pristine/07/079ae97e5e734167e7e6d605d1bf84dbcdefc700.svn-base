...
class Vote extends ServerBase
{
    ...
    public function edit($id,$data){
        try{
            ...
            if(!empty($data['fontbgcolor'])){
                $data['fontbgcolor'] = strval($data['fontbgcolor']);
            }
    ...
    public function enable($id,$status){
        $result  = $this->db->name('vote')->where('id','=',$id)->update(['status'=>$status]);
        return $result === false ? $this->responseFail():$this->responseOk();
    }
    //====================== 用户操作投票 end ======================//
}