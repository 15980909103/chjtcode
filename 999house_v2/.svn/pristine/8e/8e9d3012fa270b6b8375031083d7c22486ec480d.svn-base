var housesHotNews = (function() {
	const html = `<span class="wrap">
						<template v-for="(news,key) in list">
							<div class="nav-news" :key="key" v-if="key < showNum" @click="goInfo(news.type, news.id)">
								...
								<template v-if="news.img">
									<div class="nav-news-bottom" v-if="news.img.length > 0">
										<template  v-for="(img,num) in news.img" >
											...
											<img :src=" $http.testUrl(img)" :key="num" v-if="num < 3" >
										</template>
	...
	return {
		...
		methods: {
			goInfo( type, id ){
				this.$api.goPage('discover/news_detail.html',{ id: id, pid: 9, cate_id: 10 });
