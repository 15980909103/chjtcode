  ...
  <div class="_container">
    <div class="tb-top" >
      <el-form :inline="true" :model="searchData" class="form-serch" >
        ...
        <el-button type="danger" icon="el-icon-circle-plus-outline" @click="doEdit()">新增优惠券活动</el-button>
      </el-form>
    </div>
    <!-- 表格 -->
    <el-table class="tb-title" :data="tableData" style="width: 100%">
      ...
      <el-table-column prop="opt" label="操作" align="center">
        <template slot-scope="scope">
          <el-button type="primary" size="mini" @click="openPage({url:'/coupon/shopList',data:{activity_id: scope.row.id, region_no:scope.row.region_no }})">新增店铺</el-button>
          <el-button type="primary" size="mini" @click="showThat(scope)" >复制地址</el-button>
          <el-button type="primary" size="mini" @click="showThat1(scope)" >复制核销地址</el-button>
   ...
   <el-dialog
    ...
    >
      <el-form style="padding-right:50px;" :model="formData" ref="formData" :rules="rules">
        ...
        <el-form-item label="公众号二维码" :label-width="formLabelWidth" ref="gzh_qrcode" prop="gzh_qrcode">
          <img-upload2 ref="imgUpload" url="upload/imgUpload" :show-file-list="false" :fileList.sync="formData.gzh_qrcode" :thumb='{ isthumb:1, width:1200, height:750 }' :imgIds.sync="formData.cover_id" :uploadedImg="onUploadedImg"></img-upload2>
          <!-- <img-upload ref="imgUpload" url="upload/imgUpload" :thumb='{isthumb:1,width:750,height:750}' :default_src.sync='default_src' :uploadedImg="onUploadedImg" ></img-upload> -->
        </el-form-item>

        <el-form-item label="客服二维码" :label-width="formLabelWidth" ref="kf_qrcode" prop="kf_qrcode">
          <img-upload2 ref="imgUpload" url="upload/imgUpload" :show-file-list="false" :fileList.sync="formData.kf_qrcode" :thumb='{ isthumb:1, width:1200, height:750 }' :imgIds.sync="formData.cover_id" :uploadedImg="onUploadedImg"></img-upload2>
          <!-- <img-upload ref="imgUpload" url="upload/imgUpload" :thumb='{isthumb:1,width:750,height:750}' :default_src.sync='default_src' :uploadedImg="onUploadedImg" ></img-upload> -->
        </el-form-item>
...
export default {
 ...
 showThat(e){
     let url = this.$baseconfig.host + e.row.qd
    ...
    showThat1(e){
      let url = this.$baseconfig.host + e.row.hx
      this.$copyText(url).then(
        res => {
          this.$message({
            message: '复制成功！',
    ...
    doEdit(e={}){
      ...
      if(Object.keys(e).length>0){
        ...
        if(this.$refs.editor){//重置富文本信息
          this.$refs.editor.setContent(e.context_rule);
        }
      }
