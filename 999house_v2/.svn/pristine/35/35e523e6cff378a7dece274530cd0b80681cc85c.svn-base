		...
		methods: {
			...
			getActiveAnchorIndex() {
				...
				for (let i = this.children.length - 1; i >= 0; i--) {
					...
					if( this.showSidebar ){
						if (reachTop >= children[i].top-this.topHeight-uni.upx2px(this.moreTop)) {
							return i;
						}
					} else {
						if (reachTop >= children[i].top-this.topHeight-uni.upx2px(this.moreTop)-uni.upx2px(20)) {
							return i;
						}
					}
				}
			...
			onScroll() {
				...
				// console.log(active)
				this.activeAnchorIndex = this.scrollToAnchorIndex = active >= 0 ? active : 0;

				if( this.sendState ){
					this.$emit('active', this.activeAnchorIndex);
				} else {
					this.sendState = !this.sendState;
				}
				// console.log(this.activeAnchorIndex)
			...
			scrollToAnchor(index,type) {
				...
				// console.log(99999999999999999999,this.scrollToAnchorIndex)
				if( type == 1 ){
				...
				if (anchor) {
					// console.log(anchor.top,'---',this.scrollTop,'---',this.moreTop);
					
					this.$emit('select', this.indexList.indexOf(anchor.index));
					
					let _distance;
					
					if( this.showSidebar ){
						_distance = anchor.top + this.scrollTop - this.moreTop;
					} else {
						_distance = anchor.top + this.scrollTop - this.moreTop + 20;
					}
					
					uni.pageScrollTo({
						duration: type == 1 ? 300 : 0,
						scrollTop: _distance,
						success: function(){
							setTimeout(()=>{
								that.$emit('ok');
