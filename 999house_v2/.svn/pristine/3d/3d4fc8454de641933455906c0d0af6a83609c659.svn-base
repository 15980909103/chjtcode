...
//        var_dump($data);
        ...
        if($list['list']){
            $ids  = [];
            foreach ($list['list'] as $k => $v ){
                $data['list'][$k]['release_time']    = $this->getTimeLabel($v['release_time']) ;
                $data['list'][$k]['id']              = $v['id'];
                $data['list'][$k]['title']           = $v['name'];
                ...
                if(count($data['list'][$k]['img']) == 2){
                    $data['list'][$k]['img'] = $data['list'][$k]['img'][0] ?? [];
                }
            }
        }
//        var_dump($ids);
...
//        $comment = array_splice($comment,0,3);
        ...
        $list = [
                'info' =>[
                    ...
                    'release_time'          => $this->getTimeLabel($info['release_time']),
                    'id'            => $info['id'],
                    'likeStatus'    => !empty($is_like) ? 1 : 0,
                    'favorite'      => !empty($is_follow) ? 1 : 0,
                    'source_link'   => $info['source_link'] ?? '',
                ],
                'share_title'   => $info['name'],
                'share_context' => $info['description'],
                'share_ico'     => json_decode($info['img_path'],true)[0]['url'] ?? 'https://mo.999house.com//upload/images/admin/admin/t99735676ac125145d8e15cd4875d158a.png',
                'adv' => $adv[0] ?? [],
                'recommend' => $recommend,
                'replyList'   => $comment ??'',
        ];
...
//        var_dump($data);
    ...
    //广告点击量接口
    public function clickOnAd(){
        $param = $this->request->param();
        $res = (new Banner())->clickOnAd($param);
        if($res['code'] == 1){
            $this->success();
        }else{
            $this->error($res['msg']);
