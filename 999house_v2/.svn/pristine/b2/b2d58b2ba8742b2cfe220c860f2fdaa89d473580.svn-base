<template>
  <div class="_container">
    ...
    <el-table class="tb-title" :data="tableData" style="width: 100%">
      ...
      <el-table-column prop="type" label="版面类型"  align="center" width="100">
        <template slot-scope="scope">
            版面{{scope.row.type+1}}
        </template>
      </el-table-column>

      <el-table-column prop="type" label="时间范围" width="280" align="center">
        <template slot-scope="scope">
          <span v-if="scope.row.start_time">{{getFormatDate(scope.row.start_time)}} -- {{getFormatDate(scope.row.end_time)}}</span>
          <span v-else>永久</span>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="opt" label="操作" align="center">
        <template slot-scope="scope">
          <el-button type="primary" size="mini" @click="openPage({url:'/marketing/subjectConfig',data:{id: scope.row.id, subject_type:scope.row.type}})">模块配置</el-button>
          <el-button type="primary" size="mini" @click="openPage({url:'/marketing/signupLog',data:{signup_id: scope.row.id, region_no:scope.row.region_no,type:2 }})">报名记录</el-button>
           <!-- <el-button type="primary" size="mini" @click="showThat(scope)" >复制链接</el-button> -->
           <el-button type="primary" size="mini" @click="showThat(scope.row)" >复制链接</el-button>
   ...
   <el-row>
          <el-col :span="12">
            <el-form-item label="微信h5链接" prop="name" :label-width="formLabelWidth">
              <el-input style="width:100%" disabled v-model="wx_h5" placeholder="请输入内容"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="抖音链接" prop="name" :label-width="formLabelWidth">
              <el-input style="width:100%" disabled v-model="douyin" placeholder="请输入内容"></el-input>
            </el-form-item>
          </el-col>
      </el-row>
    </el-form>
    ...
    >
      <el-form style="padding-right:50px;" :model="formData" ref="formData" :rules="rules">
        ...
        <el-form-item label="时间范围" :label-width="formLabelWidth">
           <el-radio-group v-model="formData.time_show_status">
             <el-radio  label="0">关闭</el-radio>
             <el-radio  label="1">启用</el-radio>
           </el-radio-group>
        </el-form-item>
        <el-row v-if="time_box_show">
          <el-col :span="12">
        ...
        <el-form-item label="报名" :label-width="formLabelWidth">
           <el-radio-group v-model="formData.sign_up">
             <el-radio  label="1">启用</el-radio>
             <el-radio  label="2">禁用</el-radio>
           </el-radio-group>
        </el-form-item>

        <el-form-item label="版面选择" :label-width="formLabelWidth">
          <el-radio-group v-model="formData.type">
            <div class="radio-item" v-for="(item) in type_list" :key="item">
              <el-image class="radio-img" @click="doPreviewShow(subject_preview[item])" :src="getRealImgUrl('/static/images/admin/subject_'+item+'.jpg')"></el-image>
              <div class="radio-el"><el-radio :label="String(item)">版面{{item+1}}</el-radio></div>
            </div>
          </el-radio-group>
        </el-form-item>

        <el-form-item label="封面图" :label-width="formLabelWidth" ref="cover_url" prop="cover_url">
          <img-upload2 ref="imgUpload" url="upload/imgUpload" :thumb='{ isthumb:1, width:750, height:750 }' :limit='1' :show-file-list="false" :fileList.sync="formData.cover_url" :imgIds.sync="formData.cover_id" :uploadedImg="onUploadedImg"></img-upload2>
        
        </el-form-item>

        <el-form-item label="背景图" :label-width="formLabelWidth" ref="bg_img" prop="bg_img">
          <img-upload2 ref="imgUpload" url="upload/imgUpload" :thumb='{ isthumb:1, width:900, height:900 }' :limit='1' :show-file-list="false" :fileList.sync="formData.bg_img" :imgIds.sync="formData.cover_id" :uploadedImg="onUploadedImg"></img-upload2>
        
        </el-form-item>

        <el-form-item label="底色选择" :label-width="formLabelWidth" ref="bgcolor" prop="bgcolor">
          <el-color-picker el-color-picker v-model="formData.bgcolor"></el-color-picker>
        </el-form-item>
        <el-form-item v-if="dialogVisibleEdit" prop="context_rule" label="内容详情" required label-width="80px">
...
export default {
  ...
  　　　　return false; 
    ...
    return {
      subject_preview:{
        0: '/static/images/admin/subject_0.jpg',
        1: '/static/images/admin/subject_1.jpg',
      },
  ...
  watch:{
    'formData.time_show_status' : function(val){
      if(val=='1'){
        this.time_box_show = true
      }else{
        this.time_box_show = false
      }
    }
  },
  created: function(){
  ...
  methods:{
    ...
    // let url1 = this.$baseconfig.host + e.row.douyin
    this.wx_h5 = this.$baseconfig.host + e.wx_h5
     this.douyin = this.$baseconfig.host + e.douyin
    ...
    doEdit(e={}){
      ...
      if(Object.keys(e).length>0){
        ...
        this.formData.sign_up = String(this.formData.sign_up)
        ...
        this.formData.time_show_status = String(this.formData.time_show_status)
    ...
    doEditCancel(formName){
      ...
      if(that.dialogVisibleEdit == true){
       ...
       if(that.dialogVisibleEdit1 == true){
        that.dialogVisibleEdit1 = false
      }
