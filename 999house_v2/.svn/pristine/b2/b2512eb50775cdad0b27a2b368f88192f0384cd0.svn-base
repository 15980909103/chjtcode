				...
				<template v-if="[0,1,2,3,4,5,7,9,10].includes(item.type)">
						...
						<view class="template-top-houses" v-if="item.info.name">
							<view class="template-top-houses-name">
								<div class="templ_1">
									...
									<block v-if="item.info.tip && item.info.tip.length > 0">
										<block v-for="(houseTip,houseKey) in item.info.tip" :key="houseKey">
											<span 
					...
					>
						<h4 class="template-title">
							<template v-if="(item.type == 0 || item.type == 1) && ( item.hot == 1 || item.write == 1 ) && !item.html_title && haveTag && item.write == 1">
					...
					<view class="template-bottom" v-if="item.type != 9 && item.type != 10">
						...
						<template v-if="item.type == 0 || item.type == 1 || item.type == 5">
							...
							<view class="template-bottom-news" v-if="haveTag">
