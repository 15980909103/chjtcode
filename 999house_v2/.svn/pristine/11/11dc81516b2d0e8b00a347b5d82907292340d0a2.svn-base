				...
				<view class="u-scroll-box" :class="{'u-tabs-scorll-flex': !isScroll}">
					<view class="u-tab-item u-line-1" :id="'u-tab-item-' + index" v-for="(item, index) in list" :key="index" @tap="clickTab(index)"
					 :style="[tabItemStyle(index)]">
						...
						<span class="iconfont" :class="[getIcon(item)]"></span>
						{{ item }}
						<span v-if="item.info.length > 0">({{ item.info.length }})</span>
					</view>
		...
		computed: {
			...
			tabBarStyle() {
				let style = {
					...
					transform: `translate(${this.scrollBarLeft}px, ${this.barTrans})`,
			...
			getIcon( name ) {
				switch( name ){
					case '公交':
						return 'iconjiaotong';
					case '教育':
						return 'iconjiaoyubeifen';
					case '医院':
						return 'iconyiyuan';
					case '购物':
						return 'icongouwu';
					case '美食':
						return 'iconweibiaoti--beifen';
				}
			}
		},
		mounted() {
