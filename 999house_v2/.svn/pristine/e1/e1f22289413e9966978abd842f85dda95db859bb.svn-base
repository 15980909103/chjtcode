<template>
	<view class="content">
		<!-- <web-view :webview-styles="webviewStyles" :src='h5Host+"/discover/index.html?"+t_version'></web-view> -->
		<view style="position: relative; z-index:999">
			 <i class="iconfont icon901" @click="goPage('discover/search')"></i>
		</view>
		<!-- <van-sticky :offset-top="0" style="position: relative; z-index:999">
		  <i class="iconfont icon901" @click="$api.goPage('discover/search')"></i>
		</van-sticky> -->
		<u-tabs :list="navList" :is-scroll="false" :current="navActive" @change="navChange" active-color="#FE821E"></u-tabs>
		<van-tabs class="tabs" v-model="navActive" sticky @change="navChange">
			<van-tab v-for="( item, index ) in navList" :title="item.name" :key="index" v-show="navActive == index">
	...
	export default {
		...
		methods:{
			...
			navChange(e) {
				...
				this.onLoads()
				// this.$api.debounce(this.onLoads,500);
			},
			tagChange( e ) {
				this.tag = e;
...
<style>
	.icon901{
		width: 125rpx;
		height: 44px;
		background: linear-gradient(90deg, rgba(255, 255, 255, 0.24) 0%, #FFFFFF 100%);
		position: absolute;
		right: 0;
		top:0;
		z-index: 999;
