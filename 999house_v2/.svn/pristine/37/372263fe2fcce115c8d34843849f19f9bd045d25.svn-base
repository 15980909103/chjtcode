...
//                ->join('wechat_user w', 'unionid = unionid')
     ...
     * 判断用户是否关注订阅号
     */
    public function isFollwInfo($bind_wx_city_no,$unionid){
        if(empty($bind_wx_city_no || empty($unionid))){
           return $this->responseFail('参数错误');
        }
        $info = $this->db->name('user_association')->where('bind_wx_city_no','=',$bind_wx_city_no)
