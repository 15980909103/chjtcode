...
class TaskController extends AdminBaseController
{
     ...
     * 拉取楼盘指定字段的数据
     */
    public function setEstatesField()
    {
        try {
            $server = new Task();

            $task = $server->getTaskDispatch([
                ['name', '=', 'appointEstatesField'],
                ['is_finish', '=', '0'],
            ]);
            if(empty($task['0'])) {
                throw new Exception('任务不存在或已结束');
            ...
            $paramsEstates = [
                ...
                'offset' => $offset,
                'limit' => $limit,
                'fields' => 'id, mainlayout',
            ];

            $estates = $server->getOldData($paramsEstates);// 楼盘

            $last = 0;
            if(!empty($estates)) {
                $res = $server->setEstatesField($estates);
