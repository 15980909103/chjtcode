        ...
        try {
            ...
            if(empty($params['city'])){
                if(empty($params['lat']) || empty($params['lng'])){
                    return $this->responseFail('缺少坐标参数');
                }
                $data['boundary']  = "nearby(".$params['lat'].",".$params['lng'].",1000)";
            }else{
                if(empty($params['city'])){
                    return $this->responseFail('城市不能为空');
                }
                $data['boundary'] = "region(".$params['city'].",0)";


//                $data['filter'] = 'category<>公司企业';
            }

            if(!empty($params['filter']) && $params['filter'] != 'undefined'){
                $data['filter'] = urlencode($params['filter']);
            }
...
//            }
    ...
    //地址解析
    public function mapAddress($params){
        try {
            $url = 'https://apis.map.qq.com/ws/geocoder/v1/?address=';
            if(empty($params['address'])){
                return $this->responseFail('缺少参数');
            }
            $url = $url.$params['address'].'&key='.MyConst::MAP_KEY;
            $res = $this->curlGet($url);
            if($res['status'] != 0){
                return $this->responseFail($res['message']);
            }
            return $this->responseOk($res['result']);
        }catch (\Exception $exception){
            return $this->responseFail($exception->getMessage());
        }
    }
