...
class UserController extends UserBaseController
{
    ...
    public function decodeEncryptedData(){
        ...
        $data['session_key'] = $this->getReids()->hGet(MyConst::WXXCX_SESSION_KEY,$user_id);

        if(empty($data['encryptedData']) || empty($data['iv']) || empty($data['session_key']) ){
            return $this->error('参数错误');
        }
        $list  = [];
        (new WxMini($this->config) )->doDecryptDataForWxApp($data,$list);
    ...
    public function getInfo()
    {
        ...
        unset($res['result']['token']);
        unset($res['result']['user_name']);
        unset($res['result']['user_avatar']);
