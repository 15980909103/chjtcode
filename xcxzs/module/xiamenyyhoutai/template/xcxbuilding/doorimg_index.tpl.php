...
<div id="my_body" class="x-body">
    <xblock>
        <button class="layui-btn" onclick="click_edit('添加','/xiamenyyhoutai/xcxbuilding/doorimg_add',<?=$data['id']?>,'680','450');"><i class="layui-icon">&#xe608;</i>添加</button>
    </xblock>
...
<script>
    layui.use(['form','element','laypage'], function(){
        ...
        form.on('switch(status)', function(data){
            ...
            ajax("/xiamenyyhoutai/xcxbuilding/doorimg_status",{id:id,status:status},function(res){
                if(!res.success){
    ...
    function getPageData() {
        ...
        var title=$('#title').val();
        ajax("/xiamenyyhoutai/xcxbuilding/doorimg_page",{id:<?=$data['id']?>,title:title,curr:curr,limit:limit},function (data) {
            if(data.success){
                $("#tbody").empty();
                for(var i in data['data']){
                    ...
                    var $content = '<tr>' +
                        ...
                        '<td class="td-manage">'+
                        '<button type="button" class="layui-btn layui-btn layui-btn-xs" onclick="click_edit(\'编辑\',\'/xiamenyyhoutai/xcxbuilding/doorimg_edit\','+id+',\'680\',\'450\')" ><i class="layui-icon">&#xe642;</i>编辑</button>'+
    ...
    function click_del(id){
        layer.confirm('确认要删除吗？',function(){
            ajax("/xiamenyyhoutai/xcxbuilding/doorimg_del",{id:id},function(res){
                if(res.success){
