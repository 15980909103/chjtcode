    ...
    <form class="layui-form layui-form-pane">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <input type="text" name="title" lay-verify="required" placeholder="如：“ 1单元 ”" autocomplete="off" class="layui-input" value="<?=$data['title']?>">
            </div>
            <label class="layui-form-label">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <select name="stairs_number" lay-verify="required">
                    <?php for($i=1;$i<=30;$i++){?>
                        <option value="<?=$i?>" <?=$i==$data['stairs_number']?'selected':''?>><?=$i?>户</option>
                    <?php } ?>
                </select>
            </div>
        </div>
...
<script>
    layui.use(['form'], function(){
        ...
        form.on('submit(btn)', function(data){
            ajax("/xiamenyyhoutai/xcxbuilding/unit_doedit",data.field,function(res){
                if(res.success){
