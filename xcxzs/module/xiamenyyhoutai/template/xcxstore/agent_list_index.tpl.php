    ...
    <script>
      ...
      layui.use(['form','laypage','element'], function(){
        ...
        form.on('switch(status)', function(data){
          var id=data.value;
          var status=data.elem.checked==true?1:-1;
          ajax("/xiamenyyhoutai/xcxstore/agent_set_status",{id:id,status:status},function(res){
            if(!res.success){
      ...
      function getPageData() {
        ...
        var name=$('#name').val();
        ajax("/xiamenyyhoutai/xcxstore/agent_list_page",{nickname:nickname,name:name,curr:curr,limit:limit},function (data) {
          if(data.success){
            $("#tbody").empty();
