...
class WxNotify extends ServerBase
{
    ...
    public function getAccessToken(){
        ...
        return !empty($res['body'])&&!empty($res['body']['data'])?$res['body']['data']:'';
    }
    ...
    public function transforSendTplType($data = [],$dotype = ''){
        ...
        switch ($dotype){
            ...
            case 'expire':
                ...
                $this->db->name('log')->insert([
                    'title' => $dotype=='expire'?'过期失效通知':$this->stepTxt[$applyStep].'无效通知',
                    'content' => json_encode([
                        '$data' => $data,
                ...
                if($dotype=='expire'){
                    $tml_title = '失效通知';
                    $tml_msg = '客户'.$this->stepTxt[$applyStep].'已失效，未在保护期内，点击查看详情！';
                }else{
                    $tml_title = $this->stepTxt[$applyStep].'无效通知';
                    $tml_msg = '该客户为无效客户，点击查看详情！';
                }
                ...
                foreach ($openidList as $item){
                    $this->sendWxMsgTpl([
                        ...
                        'data' => [
                            'first'    => ['value' => $tml_title, 'color' => '#173177'],
                            ...
                            'remark'   => ['value' => $tml_msg, 'color' => '#173177']
                        ]
                    ], $accessToken);
                }
            ...
            case 'commission':
                ...
                foreach ($res_duplicate['detail_openids'] as $k=>$item){
                    $this->sendWxMsgTpl([
                        ...
                        'data' => [
                            ...
                            'keyword4' => ['value' => $applyInfo['user_phone'], 'color' => '#173177'],//客户电话
                            'keyword5' => ['value' => $applyInfo['building_name'], 'color' => '#173177'], //报备楼盘
                            'remark'   => ['value' => '客户'.$this->stepTxt[$applyStep-1].'成功，正在确认'.$this->stepTxt[$applyStep].'，点击查看详情！', 'color' => '#173177']
                        ]
                    ], $accessToken);
                }
                ...
                $this->sendWxMsgTpl([
                     ...
                     'data' => [
                         ...
                         'remark'   => ['value' => "结佣已完成，点击查看详情！", 'color' => '#173177']
                     ]
                 ], $accessToken);
                break;
            case 'log':
                ...
                foreach ($openidList as $item){
                    $this->sendWxMsgTpl([
                        ...
                        'data' => [
                            ...
                            'keyword4' => ['value' => $applyInfo['user_phone'], 'color' => '#173177'],//客户电话
                            'keyword5' => ['value' => $applyInfo['building_name'], 'color' => '#173177'], //报备楼盘
                            'remark'   => ['value' => '您有新的有新的进信息，点击查看详情！', 'color' => '#173177']
                        ]
                    ], $accessToken);
                }
    ...
    public function getExamineOpenidsOrSaids($roles_examine, $info, $key='openid'){
        ...
        if(!empty($roles_examine['detail_roleIds']['create-store'])||!empty($roles_examine['detail_roleIds']['subordinate-store'])){
            ...
            if(!empty($roles_examine['detail_roleIds']['create-store'])){
                $create_store_agents = $this->db->name('xcx_store_store')
                    ->alias('xs')
                    ->field('xa.said,xa.agent_openid,xa.mgid,xa.agent_id')
                    ->join('admin a','a.id = xs.aid')
                    ->join('xcx_store_agent xa','xa.said = a.channel_id')
                    ->where([
                        ['xs.id', '=', $info['store_id']],
                        ['xa.type', 'in', $roles_examine['detail_roleIds']['create-store']]
                    ])->find();
                //剔除自己操作的申请，不进行通知
                if(!empty($create_store_agents['agent_id'])&&!empty($create_store_agents['said'])&&$info['said']!=$create_store_agents['said']){
            ...
            /*if(!empty($roles_examine['detail_roleIds']['subordinate-store'])&&!empty($create_store_agents['mgid'])){
                $leader_create_store_agents = $this->db->name('xcx_store_agent')
                    ->field('said,agent_openid,agent_id')
                    ->where(
                        ['mgid', '=', $create_store_agents['mgid']],
                        ['type', 'in', $roles_examine['detail_roleIds']['subordinate-store']]
                    )->find();
                //剔除自己操作的申请，不进行通知
                if(!empty($leader_create_store_agents['agent_id'])&&!empty($leader_create_store_agents['said'])&&$info['said']!=$leader_create_store_agents['said']){
                    $leader_create_store_agents['agent_openid']&&$openIds[$leader_create_store_agents['agent_openid']] = $leader_create_store_agents['agent_openid'];
        ...
        if(!empty($roles_examine['detail_roleIds']['create-building'])){
            ...
            $create_building_agents = $this->db->name('xcx_store_agent')
                ->alias('xa')
                ->field('xa.said,xa.agent_openid,xa.agent_id')
            ...
            //剔除自己操作的申请，不进行抄送
            if(!empty($create_building_agents['agent_id'])&&!empty($create_building_agents['said'])&&$info['said']!=$create_building_agents['said']){
                $create_building_agents['agent_openid']&&$openIds[$create_building_agents['agent_openid']] = $create_building_agents['agent_openid'];
                $create_building_agents['agent_openid']&&$detail_openIds['create-building'][$create_building_agents['agent_openid']] = $create_building_agents['agent_openid'];
        ...
        if(!empty($roles_examine['detail_roleIds']['building'])){
            ...
            //剔除自己操作的申请，不进行通知
            if(!empty($bind_building_agents['agent_id'])&&!empty($bind_building_agents['said'])&&$info['said']!=$bind_building_agents['said']){
                $bind_building_agents['agent_openid']&&$openIds[$bind_building_agents['agent_openid']] = $bind_building_agents['agent_openid'];
                $bind_building_agents['agent_openid']&&$detail_openIds['building'][$bind_building_agents['agent_openid']] = $bind_building_agents['agent_openid'];
    ...
    public function getDuplicateOpenidsOrSaids($roles_duplicate, $info){
        ...
        if(!empty($roles_duplicate['detail_roleIds']['city'])){
            ...
            foreach ($city_agents as $item){
                if(!empty($item['agent_id'])&&$info['said']!=$item['said']){
                    $item['agent_openid']&&$openIds[$item['agent_openid']] = $item['agent_openid'];
                    $item['agent_openid']&&$detail_openIds['city'][$item['agent_openid']] = $item['agent_openid'];
        ...
        if(!empty($roles_duplicate['detail_roleIds']['subordinate'])){
            ...
            //剔除店长自己操作的申请，不进行店长抄送
            if(!empty($store_agents['agent_id'])&&!empty($store_agents['said'])&&$info['said']!=$store_agents['said']){
                $store_agents['agent_openid']&&$openIds[$store_agents['agent_openid']] = $store_agents['agent_openid'];
                $store_agents['agent_openid']&&$detail_openIds['subordinate'][$store_agents['agent_openid']] = $store_agents['agent_openid'];
        ...
        if(!empty($roles_duplicate['detail_roleIds']['create-store'])||!empty($roles_duplicate['detail_roleIds']['subordinate-store'])){
            ...
            if(!empty($roles_duplicate['detail_roleIds']['create-store'])){
                $create_store_agents = $this->db->name('xcx_store_store')
                    ...
                    ->where([
                        ['xs.id', '=', $info['store_id']],
                        ['xa.type', 'in', $roles_duplicate['detail_roleIds']['create-store']]
                    ])->find();
                //剔除自己操作的申请，不进行抄送
                if(!empty($create_store_agents['agent_id'])&&!empty($create_store_agents['said'])&&$info['said']!=$create_store_agents['said']){
                    $create_store_agents['agent_openid']&&$openIds[$create_store_agents['agent_openid']] = $create_store_agents['agent_openid'];
            ...
            if(!empty($roles_duplicate['detail_roleIds']['subordinate-store'])&&!empty($create_store_agents['mgid'])){
                ...
                if(!empty($leader_create_store_agents['agent_id'])&&!empty($leader_create_store_agents['said'])&&$info['said']!=$leader_create_store_agents['said']){
                    $leader_create_store_agents['agent_openid']&&$openIds[$leader_create_store_agents['agent_openid']] = $leader_create_store_agents['agent_openid'];
                    $leader_create_store_agents['agent_openid']&&$detail_openIds['subordinate-store'][$leader_create_store_agents['agent_openid']] = $leader_create_store_agents['agent_openid'];
                    $saIds[$leader_create_store_agents['said']] = $leader_create_store_agents['said'];
                    $detail_saIds['subordinate-store'][$leader_create_store_agents['said']] = $leader_create_store_agents['said'];
                }
            }
        ...
        if(!empty($roles_duplicate['detail_roleIds']['building'])||!empty($roles_duplicate['detail_roleIds']['subordinate-building'])){
            ...
            if(!empty($roles_duplicate['detail_roleIds']['subordinate-building'])&&!empty($bind_building_agents['mgid'])){
                ...
                //剔除自己操作的申请，不进行抄送
                if(!empty($leader_bind_building_agents['agent_id'])&&!empty($leader_bind_building_agents['said'])&&$info['said']!=$leader_bind_building_agents['said']){
                    $leader_bind_building_agents['agent_openid']&&$openIds[$leader_bind_building_agents['agent_openid']] = $leader_bind_building_agents['agent_openid'];
                    $leader_bind_building_agents['agent_openid']&&$detail_openIds['subordinate-building'][$leader_bind_building_agents['agent_openid']] = $leader_bind_building_agents['agent_openid'];
        ...
        if(!empty($roles_duplicate['detail_roleIds']['subordinate-work'])){
            ...
            $sub_agent = $this->db->name('xcx_store_agent')
                ->alias('xa')
                ->field('xa.said,xa.agent_openid,xa.mgid,xa.type,xa.agent_id')
            ...
            if(!empty($sub_agent['type'])&&!in_array($sub_agent['type'],$roles_duplicate['detail_roleIds']['subordinate-work'])){
                ...
                if(!empty($leader_examine_work_agents['agent_id'])&&!empty($leader_examine_work_agents['said'])&&$info['said']!=$leader_examine_work_agents['said']){
                    $leader_examine_work_agents['agent_openid']&&$openIds[$leader_examine_work_agents['agent_openid']] = $leader_examine_work_agents['agent_openid'];
                    $leader_examine_work_agents['agent_openid']&&$detail_openIds['subordinate-work'][$leader_examine_work_agents['agent_openid']] = $leader_examine_work_agents['agent_openid'];
