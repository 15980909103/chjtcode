...
class Menu
{
     ...
     * @return array
     */
    public function init($arr = [],$rules = [])
    {
        $this->arr = $arr;
        $this->rules = $rules;
        $this->ret = '';
    ...
    public function getChild($myId)
    {
        ...
        if (is_array($this->arr)) {
            foreach ($this->arr as $id => $a) {
                if(!empty($this->rules)){
                    if ($a['parent_id'] == $myId && in_array($a['url'],$this->rules)) {
//                    unset($a['status']);
    ...
    /**
     * 得到树型结构
     * @param int ID，$myId 表示获得这个ID下的所有子级
     ...
     * @return array
     */
    public function getTreeArrayForReIndex($array){
        $returnArray = [];
        if (!empty($array)&&is_array($array)) {
            $returnArray = array_values($array);
            foreach ($returnArray as &$item) {
                if (!empty($item['children'])&&is_array($item['children'])) {
                    $item["children"] = $this->getTreeArrayForReIndex($item['children']);
                }
            }
        }
