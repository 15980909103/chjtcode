            ...
            <el-table :data="item.chunkList" border max-height="300">
              ...
              <el-table-column label="大小" align="center" width="120">
                <template slot-scope="scope">
                  {{ scope.row.size | transformByte }}
                </template>
              </el-table-column>
              <el-table-column prop="uploaded" label="是否完成" align="center">
                <template slot-scope="scope">
                  {{ scope.row.uploaded ? '完成' : '进行中' }}
                </template>
              </el-table-column>

              <el-table-column label="进度" align="center">
                <template slot-scope="scope">
                  <el-progress v-if="!scope.row.status || scope.row.status === 'wait'" :percentage="scope.row.progress" />
                  <el-progress v-else :percentage="scope.row.progress" :status="scope.row.status" />
                </template>
              </el-table-column>
            </el-table>
          </div>
    ...
    methods: {
      ...
      handleStart(rawFile) {
        ...
        if (this.beforeUpload) {
          ...
          if(this.uploadFiles.length >=this.limitnum){
            this.$message.warning(
              `最多添加${this.limitnum}个文件`
            );
            return;
          }
        ...
        return new Promise((resolve, reject) => {
          const handler = () => {
            ...
            if (forms.length) {
              ...
              instance
                .post('upload/uploadSimple', formData, {
      ...
      verifyUpload(fileName, fileHash) {
        return new Promise((resolve) => {
          ...
          instance
            .get('upload/presence', { params: obj })
            .then((res) => {
              console.log('verifyUpload -> res', res);
              if(res.data.code ==2000 && res.data.data.presence) {
