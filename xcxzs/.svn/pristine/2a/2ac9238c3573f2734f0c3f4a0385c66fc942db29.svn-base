...
class CommentController extends UserBaseController
{
    ...
    public function propertyReviewsComment()
    {
        ...
        try {
            ...
            unset($img,$item);
            $miniServer = new WxMini(Config::get('app.miniconfig'));

            foreach ($new_img as $v){
                $miniServer->checkWxImgNew($v);
            }
            $miniServer->checkWxText($comment);

            $type = $this->deviceType;
            if(!empty($pid) && !empty($user_id)){
                if($pid == $user_id){
                    throw new \Exception('用户本身不能回复自己的回复');
                }
            }
            $param = [
                'id'       => $id,
    ...
    public function newsComment(){
        ...
        $server  = new WxMini(Config::get('app.miniconfig'));
        try {
              $resutl = $server->checkWxText($comment);
