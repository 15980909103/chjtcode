...
<div class="x-body">
    <form class="layui-form layui-form-pane">
            ...
            <div class="layui-input-block">
                <select name="aid" lay-verify="required" lay-search>
                    <?php foreach($articleData as $val){ ?>
                        <option value="<?=$val['id']?>" <?=$val['id']==$data['aid']?'selected':''?>><?=$val['title']?></option>
                    <?php } ?>
                </select>
            </div>
        </div>
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="radio" name="user_type" value="1" lay-filter="usertype" title="小程序用户" <?=$data['user_type']=='1'?'checked':''?>>
                <input type="radio" name="user_type" value="2" lay-filter="usertype" title="经纪人" <?=$data['user_type']=='2'?'checked':''?>>
            </div>
        </div>
        <div id="xcxuser" class="layui-form-item" style="display:<?=$data['user_type']=='1'?'block':'none';?>;">
            <label class="layui-form-label">
            ...
            <div class="layui-input-block">
                <select name="uid" lay-verify="required" lay-search>
                    <?php foreach($userData as $val){ ?>
                        <option value="<?=$val['id']?>" <?=($val['id']==$data['uid'] && $data['user_type']=='1')?'selected':''?>><?=$val['nickName']?></option>
                    <?php } ?>
                </select>
            </div>
        </div>
        <div id="jjruser" class="layui-form-item" style="display:<?=$data['user_type']=='2'?'block':'none';?>;">
            <label class="layui-form-label">
            ...
            <div class="layui-input-block">
                <select name="uid" lay-verify="required" lay-search>
                    <?php foreach($agentData as $val){ ?>
                        <option value="<?=$val['id']?>" <?=($val['id']==$data['uid'] && $data['user_type']=='2')?'selected':''?>><?=$val['nickname']?></option>
                    <?php } ?>
                </select>
            </div>
        </div>
...
<script>
    layui.use(['form'], function(){
        ...
        form.on('submit(btn)', function(data){
            ajax("/xiamenyyhoutai/xcxarticle/comments_doedit",data.field,function(res){
                if(res.success){
