    ...
    <form class="layui-form layui-form-pane">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <input type="text" name="title" autocomplete="off" maxlength="250" class="layui-input" value="<?=$data['title'];?>">
            </div>
            <label class="layui-form-label">
                <span class="x-red">*</span>排序
            </label>
            <div class="layui-input-inline">
                <input type="number" name="sort" required="" lay-verify="required" min="0" autocomplete="off" class="layui-input" value="<?=$data['sort'];?>">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <img id="mylogoimg" class="myimg" src="<?=$data['img']?>" alt="暂无图片" style="max-width: 100%;">
            </div>
        </div>
...
<script>
    ...
    layui.use(['form','upload'], function(){
        ...
        var logoUpload=upload.render({
            ...
            ,url: "/xiamenyyhoutai/xcxarticle/figure_doedit" //上传接口
            ,size: 2048
            ,accept: "images"
            ,auto:false
            ,data:{
                ...
                sort: function(){
                    return  document.getElementsByName("sort")[0].value;
                },
                url: function(){
        ...
        form.on('submit(btn)', function(data){
            if(fileIndex==""){
                ...
                ajax("/xiamenyyhoutai/xcxarticle/figure_doedit",parameter,function(res){
                    if(res.success){
                        parent.layer.alert("保存成功", {icon: 6},function (index2) {
                            parent.getPageData(parent.$('#my_body').data('curr'));
                            parent.layer.close(index2);
