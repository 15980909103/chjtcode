...
class DataBase {
    ...
    public static function initConnection() {
        try {
            if (!static::$conn) {
                static::$conn = new PDO('mysql:host=' . MYSQL_HOST . ';port=' . MYSQL_PORT . ';charset=' . MYSQL_CHARSET . ';dbname=' . MYSQL_DB, MYSQL_USER, MYSQL_PASSWORD);
                static::$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                static::exec("set NAMES 'utf8mb4'");
            }
        } catch (Exception $ex) {
            die($ex->getMessage());
			//die("a");
