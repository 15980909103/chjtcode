...
<html>
  ...
  <body class="layui-anim layui-anim-up">
    ...
    <div id="my_body" class="x-body">
      ...
      <xblock>
        <button class="layui-btn" onclick="click_add('添加','/xiamenyyhoutai/xcxstore/store_add','350','450')"><i class="layui-icon">&#xe608;</i>添加</button>
      </xblock>
    ...
    <script>
      ...
      layui.use(['form','laypage','element'], function(){
        ...
        form.on('switch(status)', function(data){
          ...
          ajax("/xiamenyyhoutai/xcxstore/store_status",{id:id,status:status},function(res){
            if(!res.success){
      ...
      function getPageData() {
        ...
        ajax("/xiamenyyhoutai/xcxstore/store_page",{title:title,status:status,curr:curr,limit:limit},function (data) {
          if(data.success){
            $("#tbody").empty();
            for(var i in data['data']){
              ...
              var $content = '<tr>' +
                      ...
                      '<td>'+data['data'][i]['main_area']+'</td>' +
                      '<td>'+status+'</td>' +
                      '<td>'+data['data'][i]['create_time']+'</td>' +
                      '<td>'+data['data'][i]['update_time']+'</td>' +
                      '<td class="td-manage">'+
                      '<button type="button" class="layui-btn layui-btn layui-btn-xs" onclick="click_edit(\'人员管理\',\'/xiamenyyhoutai/xcxstore/store_agent_index\','+id+',\'1000\')" ><i class="layui-icon">&#xe66f;</i>人员管理</button>'+
                      '<button type="button" class="layui-btn layui-btn layui-btn-xs" onclick="click_edit(\'编辑\',\'/xiamenyyhoutai/xcxstore/store_edit\','+id+',\'350\',\'450\')" ><i class="layui-icon">&#xe642;</i>编辑</button>'+
