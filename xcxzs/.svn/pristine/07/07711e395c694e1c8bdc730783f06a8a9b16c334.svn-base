...
(function(global, factory){
	...
	Calendar.prototype = {
		...
		toggle : function(mydate){
			if(this.activeView.name === 'month'){
				this.activeView = this.weekClass;
				//周日历开始为周日
				var tmpDate=new Date(mydate.year+'/'+mydate.month+'/'+mydate.day);
				var tmpxq=tmpDate.getDay();
				if(tmpxq>0){
					tmpDate.setDate(tmpDate.getDate() - tmpxq);
					mydate={year:tmpDate.getFullYear(),month:tmpDate.getMonth()+1,day:tmpDate.getDate()};
				}
			}else if(this.activeView.name === 'week'){
				this.activeView = this.monthClass;
			}
			...
			if(year < originalYear) classname = 'i-before';
				...
				if(month === originalMonth){
					if(day < originalDay) classname = 'i-before';
						...
						classname = isSigned ? 'i-signed i-today i-tick' : 'i-today i-tick';
					}
		...
		render : function(boole){
			...
			for(; i < row; i++){
				...
				for(j=0; j<7; j++){
					...
					if(day > 0 && day <= maxDay){
						html += '<td data-time="'+year+'-'+month+'-'+day+'" data-year="'+year+'" data-month="'+month+'" data-day="'+day+'" class="i-day">' + '<span class="'+this.addClass(originalYear, originalMonth, originalDay, year, month, day, isSigned)+'">'+day +'</span>'+ '</td>';
					}else{
						html += '<td></td>';
					}
				}
	...
	extend(CalendarWeek.prototype, {
		...
		render : function(boole){
			...
			weekTd2="";
			for(var i=0; i<7; i++){
				weekTd2+="<div>" + this.weekNames[i + weekDay] + "</div>";
			...
			for(i=0; i<7; i++){
				today = weeks[i];
				html += '<td data-time='+today.year+'-'+today.month+'-'+today.day+' data-year="'+today.year+'" data-month="'+today.month+'" data-day="'+today.day+'" class="i-day">' + '<span class="'
