    ...
    <form id="myForm" class="layui-form x-center" style="width:100%" action="javascript:void(0);">
        <div class="layui-form-pane" style="margin-top: 15px;">
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline">
                    <select id="matter_type" lay-verify="required">
                        <option value=""></option>
                        <option value="1">基本信息</option>
                        <option value="2">建筑信息</option>
                        <option value="3">物业参数</option>
                        <option value="4">配套信息</option>
                    </select>
                </div>
...
<script>
    ...
    layui.use(['form','laypage','element'], function(){
        ...
        form.on('switch(status)', function(data){
            ...
            ajax("/xiamenyyhoutai/xcxambitus/building_correct_building_status",{id:id,dispose_status:dispose_status},function(res){
                if(!res.success){
    ...
    function getPageData() {
        ...
        var matter_type      = $('#matter_type').val();
        ajax("/xiamenyyhoutai/xcxambitus/building_correct_page",{matter_type:matter_type,client_side_type:client_side_type,dispose_status:dispose_status,curr:curr,limit:limit},function (data) {
            if(data.success){
                console.log(data);
                $("#tbody").empty();
                for(var i in data['data']){
                    ...
                    var $content = '<tr>' +
                        ...
                        '<td>'+data['data'][i]['building_correct_info']+'</td>' +
                        '<td>'+data['data'][i]['matter_type']+'</td>' +
