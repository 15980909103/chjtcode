...
<div class="x-body">
    <xblock>
        <button class="layui-btn" onclick="click_edit('添加','/xiamenyyhoutai/xcxbuilding/door_add',<?=$data['id']?>,'680','450');"><i class="layui-icon">&#xe608;</i>添加</button>
    </xblock>
...
<script>
    layui.use(['form','element','laypage'], function(){
        ...
        form.on('switch(status)', function(data){
            ...
            ajax("/xiamenyyhoutai/xcxbuilding/door_status",{id:id,status:status},function(res){
                if(!res.success){
        ...
        form.on('switch(is_hot)', function(data){
            ...
            ajax("/xiamenyyhoutai/xcxbuilding/door_is_hot",{id:id,is_hot:is_hot},function(res){
                if(!res.success){
    ...
    function getPageData() {
        ...
        ajax("/xiamenyyhoutai/xcxbuilding/door_page",{id:<?=$data['id']?>,keyword:keyword,curr:curr,limit:limit},function (data) {
            if(data.success){
                $("#tbody").empty();
                for(var i in data['data']){
                    ...
                    var $content = '<tr>' +
                        ...
                        '<td><img class="my-img" src="'+data['data'][i]['pic']+'" alt="暂无图片" /></td>' +
                        '<td>'+data['data'][i]['title']+'</td>' +
                        '<td>'+data['data'][i]['sales_status']+'</td>' +
                        '<td>'+data['data'][i]['house_type']+'</td>' +
                        '<td>'+data['data'][i]['family_structure']+'</td>' +
                        '<td>'+data['data'][i]['construction_area']+'m²</td>' +
                        '<td>'+data['data'][i]['orientation']+'</td>' +
                        '<td>'+data['data'][i]['price_total']+'</td>' +
                        '<td>'+data['data'][i]['sort']+'</td>' +
                        '<td>'+is_hot+'</td>' +
                        '<td>'+status+'</td>' +
                        '<td>'+data['data'][i]['create_time']+'</td>' +
                        '<td class="td-manage">'+
                        '<button type="button" class="layui-btn layui-btn-normal layui-btn-xs" onclick="onUnitInfo('+'\''+data['data'][i]['title']+'轮播图\''+','+id+')" ><i class="layui-icon">&#xe705;</i>轮播图管理</button>'+
                        '<button type="button" class="layui-btn layui-btn layui-btn-xs" onclick="click_edit(\'编辑\',\'/xiamenyyhoutai/xcxbuilding/door_edit\','+id+',\'680\',\'450\')" ><i class="layui-icon">&#xe642;</i>编辑</button>'+
    ...
    function click_del(id){
        layer.confirm('确认要删除吗？',function(){
            ajax("/xiamenyyhoutai/xcxbuilding/door_del",{id:id},function(res){
                if(res.success){
    ...
    function onUnitInfo(title,id){
        parent.addCardItem(title,"/xiamenyyhoutai/xcxbuilding/doorimg_index?id="+id,id);
    }
</script>