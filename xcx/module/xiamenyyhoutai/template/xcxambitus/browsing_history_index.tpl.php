...
<div id="my_body" class="x-body">
    <form class="layui-form x-center" style="width:100%" action="javascript:void(0);">
        <div class="layui-form-item">
            ...
            <label class="layui-form-label">浏览类型</label>
            <div class="layui-input-inline">
                <select id="browse_type" lay-verify="required">
                    <option value=""></option>
...
<script>
    ...
    function getPageData() {
        ...
        ajax("/xiamenyyhoutai/xcxambitus/browsing_history_page",{browse_type:browse_type,user_id:user_id,curr:curr,limit:limit},function (data) {
            if(data.success){
                $("#tbody").empty();
                for(var i in data['data']){
                    var $content = '<tr>' +
                        ...
                        '<td>'+data['data'][i]['browse_type']+'</td>' +
                        '<td>'+data['data'][i]['start_time']+'</td>' +
                        '<td>'+data['data'][i]['end_time']+'</td>' +
                        '<td>'+data['data'][i]['viewing_hours']+'</td>' +
                        '</tr>';
