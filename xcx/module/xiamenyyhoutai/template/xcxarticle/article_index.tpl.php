...
<html>
  ...
  <body class="layui-anim layui-anim-up">
    ...
    <div id="my_body" class="x-body">
      <form id="myForm" class="layui-form x-center" style="width:100%" action="javascript:void(0);">
        <div class="layui-form-pane" style="margin-top: 15px;">
          <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
              <select id="ishot" lay-verify="required">
                <option value=""></option>
      ...
      <xblock>
        <button class="layui-btn" onclick="click_add('添加','/xiamenyyhoutai/xcxarticle/article_add','680','700')"><i class="layui-icon">&#xe608;</i>添加</button>
      </xblock>
    ...
    <script>
      ...
      layui.use(['form','laypage','element'], function(){
        ...
        form.on('switch(status)', function(data){
          ...
          ajax("/xiamenyyhoutai/xcxarticle/article_status",{id:id,status:status},function(res){
            if(!res.success){
        ...
        form.on('switch(is_hot)', function(data){
          ...
          ajax("/xiamenyyhoutai/xcxarticle/article_ishot",{id:id,is_hot:is_hot},function(res){
            if(!res.success){
              layer.msg('推荐修改失败', {icon: 5});
              $(data.elem).prop('checked',!$(data.elem).prop('checked'));
              form.render('checkbox');
            }
          });
      ...
      function getPageData() {
        ...
        var is_hot=$('#ishot').val();
        ajax("/xiamenyyhoutai/xcxarticle/article_page",{title:title,status:status,is_hot:is_hot,curr:curr,limit:limit},function (data) {
          if(data.success){
            $("#tbody").empty();
            for(var i in data['data']){
              ...
              var $content = '<tr>' +
                      ...
                      '<td><img class="my-img" src="'+data['data'][i]['aimg']+'" alt="暂无图片">'+'</td>' +
                      '<td>'+data['data'][i]['aname']+'</td>' +
                      '<td>'+data['data'][i]['cname']+'</td>' +
                      '<td><img class="my-detail-img" src="'+data['data'][i]['cover']+'" alt="暂无图片">'+'</td>' +
                      '<td>'+data['data'][i]['title']+'</td>' +
                      '<td>'+data['data'][i]['read_num']+'</td>' +
                      '<td>'+is_hot+'</td>' +
                      '<td>'+status+'</td>' +
                      '<td>'+data['data'][i]['create_time']+'</td>' +
                      '<td>'+data['data'][i]['update_time']+'</td>' +
                      '<td class="td-manage">'+
                      '<button type="button" class="layui-btn layui-btn layui-btn-xs"  onclick="click_edit(\'编辑\',\'/xiamenyyhoutai/xcxarticle/article_edit\','+id+',\'680\',\'700\')" ><i class="layui-icon">&#xe642;</i>编辑</button>'+
      ...
      function click_del(id){
        layer.confirm('确认要删除吗？',function(){
          ajax("/xiamenyyhoutai/xcxarticle/article_del",{id:id},function(res){
            if(res.success){
