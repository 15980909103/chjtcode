  ...
  <body class="layui-anim layui-anim-up">
    ...
    <script>
      ...
      function getPageData() {
        ...
        ajax("/xiamenyyhoutai/xcxstore/store_agent_page",{mgid:mgid,curr:curr,limit:limit},function (data) {
          if(data.success){
            $("#tbody").empty();
            for(var i in data['data']){
              ...
              var $content = '<tr>' +
                      ...
                      '<button type="button" class="layui-btn layui-btn-normal layui-btn-xs my-copy" onclick="buildBind('+id+')" ><i class="layui-icon">&#xe64c;</i>楼盘绑定</button>'+
                      '<button type="button" class="layui-btn layui-btn-normal layui-btn-xs my-copy" onclick="powerBind('+id+')" ><i class="layui-icon">&#xe64c;</i>权限绑定</button>'+
                      mydelbtn+
                      '</td>' +
                      '</tr>';
      ...
      function my_addag(){
        layer.confirm('确认要添加吗？',function(){
          ajax("/xiamenyyhoutai/xcxstore/store_agent_add",{mgid:"<?=$data['mgid']?>"},function(res){
            if(res.success){
      ...
      function buildBind(id) {
          click_edit('编辑','/xiamenyyhoutai/xcxstore/manager_edit_bind',id,'680','650');
      }
