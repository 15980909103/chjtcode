...
<html>
  ...
  <body class="layui-anim layui-anim-up">
    ...
    <div id="my_body" class="x-body">
      <xblock>
        <button class="layui-btn" onclick="click_add('添加','/xiamenyyhoutai/xcxarticle/figure_add','680','700')"><i class="layui-icon">&#xe608;</i>添加</button>
      </xblock>
    ...
    <script>
      ...
      layui.use(['form','laypage','element'], function(){
        ...
        form.on('switch(status)', function(data){
          ...
          ajax("/xiamenyyhoutai/xcxarticle/figure_status",{id:id,status:status},function(res){
            if(!res.success){
      ...
      function getPageData() {
        ...
        ajax("/xiamenyyhoutai/xcxarticle/figure_page",{curr:curr,limit:limit},function (data) {
          if(data.success){
            $("#tbody").empty();
            for(var i in data['data']){
              ...
              var $content = '<tr>' +
                      ...
                      '<td class="td-manage">'+
                      '<button type="button" class="layui-btn layui-btn layui-btn-xs"  onclick="click_edit(\'编辑\',\'/xiamenyyhoutai/xcxarticle/figure_edit\','+id+',\'680\',\'700\')" ><i class="layui-icon">&#xe642;</i>编辑</button>'+
      ...
      function click_del(id){
        layer.confirm('确认要删除吗？',function(){
          ajax("/xiamenyyhoutai/xcxarticle/figure_del",{id:id},function(res){
            if(res.success){
