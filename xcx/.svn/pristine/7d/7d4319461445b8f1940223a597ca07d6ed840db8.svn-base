...
require_once (Extend.DS."api_sdk/vendor/autoload.php");
...
class Sms{
    ...
    public function message($phone,$number){
        // 调用示例：
        header('Content-Type: text/plain; charset=utf-8');
        $sms = [];
        $configKey = ['accessKeyId','accessKeySecret','setSignName','setTemplateCode'];
        $query = new Query();
        foreach($configKey as &$return){
            $sms[$return] = $query->Name('setting')->where_equalTo('`key`',$return)->select('value')->firstColumn();
        }
        $this->key(
            $sms['accessKeyId'],
            $sms['accessKeySecret']
        ...
        $response = $this->sendSms(
            $sms['setSignName'], // 短信签名
            $sms['setTemplateCode'], // 短信模板编号
            $phone, // 短信接收者
            Array(  // 短信模板中字段的值
                "number"=>$number,
            )
...
//        $response = $this->queryDetails(
//            $phone,  // phoneNumbers 电话号码
//            date('Y-m-d H:i:s'), // sendDate 发送时间
//            10, // pageSize 分页大小
//            1 // currentPage 当前页码
...
//        print_r($response);
     ...
     */
    public function key($accessKeyId, $accessKeySecret)
    {
