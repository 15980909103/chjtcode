        ...
        <div class="my-div-content layui-form" lay-filter="my-radio" v-show="!is_all" style="display: none;">
            <div class="my-list-li">
                <template v-for="(item,index) in list">
                    <label  class="my-label" ><input type="radio" name="buildingname" :data-index="index" :value="item.id" lay-ignore>{{item.name}} {{item.is_show==1 ? "" : "(已被绑定)"}}</label>
                </template>
            </div>
            ...
            layui.use(['form'], function(){
                ...
                form.on('submit(btn)', function(data){
                    ...
                    layer.confirm('确认要修改吗？',function(){
                        ajax("/xiamenyyhoutai/xcxstore/manager_doedit_bind",{id:_id,building_ids:building_ids},function(res){
                            if(res.success){
        ...
        methods:{
            onGoRight(){
                ...
                if(index!=undefined){
                    ...
                    for (var i in this.resList){
                        if(this.list[index].id==this.resList[i].id){
                            has=1
                        }
                    }
                    if(has==0){
                        this.resList.push(this.list[index]);
                    }
            ...
            onGoLeft(){
                ...
                if(index!=undefined){
                    ...
                    for (var i in this.list){
                        if(this.resList[index].id==this.list[i].id){
                            has=1
                        }
                    }
                    if(has==0){
                        this.list.push(this.resList[index]);
                    }

                    this.resList.splice(index,1);
                    for(var i in this.list){
                        this.list[i].is_show = 1;
                        for(var j in this.resList){
                            if(this.list[i].id == this.resList[j].id) {
            ...
            onSearch(){
                ...
                ajax("/xiamenyyhoutai/xcxstore/searchBuilding",{name:_this.buildingText},function(res){
                    if(res.success){
                        _this.list=res.data;
                        for(var i in _this.resList){
                            for(var j in _this.list){
                                if(_this.list[j].id == _this.resList[i].id) {
                                    _this.list[j].is_show = 0;
                                } else {
                                    _this.list[j].is_show = 1;
                                }
                            }
                        }
