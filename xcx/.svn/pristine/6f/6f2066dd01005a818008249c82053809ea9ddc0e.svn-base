 ...
 * //调用示例 * $config = $RedisPoolObj->setConfig('swoole.pool.redis',$newconfig); //先获取当前配置后注入参数，避免并发时变量污染//新设配置时
...
class RedisPool
{
    ...
    public function setConfig(string $name = 'swoole.pool.redis',$newconfig = []){//新设配置时
        return $this->getConnectionConfig($name,$newconfig);
    }
    /**
     * 创建redis连接实例
     * @param bool $force 强制重新连接
     * @param array|int $config 连接的配置['database'=>选择redis库,'max_active'=>最大连接数量,...] 拓展个性化配置覆盖
     * @return \Redis|void
     */
    public function init(array $config = [], bool $force = false)
    {
        if(empty($config)){//空时使用默认配置
            //throw new \InvalidArgumentException(sprintf('Failed to connect Redis server: %s', '参数缺失'));
            $config = $this->getConnectionConfig('swoole.pool.redis', []);
        }
    ...
    public function initByFlag($flag, bool $force = false){
        $configlist = [
            0=>[],
            1=>[],
        ];
        $config = $this->getConnectionConfig($flag, $configlist[$flag]);
        if (!$config['enable']) {//是否开启链接池
            return;
        }
        $flag_name = $config['_flag_name']; //带上标识
    ...
    protected function createConnection($config)
    {
        ...
        $ret = $connection->connect($config['host'], intval($config['port']), $config['timeout'] ?? 10);
        if ($ret === false) {
