...
<script>
    ...
    function getPageData() {
        ...
        ajax("/xiamenyyhoutai/xcxbuilding/building_settlement_page",{status_type:status_type,building_id:building_id,agent_id:agent_id,user_name:user_name,user_gender:user_gender,admin_is_read:admin_is_read,curr:curr,limit:limit},function (data) {
            if(data.success){
                var domain = 'http://'+window.location.host;
                $("#tbody").empty();
                for(var i in data['data']){
                    var myStatusBtn="";
                    if(data['data'][i]['examine_type']==1){
                        var _statusTypeName="";
                        switch(data['data'][i]['examine_type']){
                            case '1':
                                _statusTypeName='结佣';
                                break;
                        }
                        myStatusBtn='<button type="button" class="layui-btn layui-btn layui-btn-xs"onclick="click_edit(\'确认结佣\',\'/xiamenyyhoutai/xcxbuilding/settlement_edit\','+data['data'][i]['id']+',\'350\',\'350\')" ><i class="layui-icon">&#xe66e;</i>'+_statusTypeName+'</button>';
                        //myStatusBtn='<button type="button" class="layui-btn layui-btn-danger layui-btn-xs" onclick="onSetStatusType('+data['data'][i]['examine_type']+','+data['data'][i]['id']+')"><i class="layui-icon">&#xe66e;</i>'+_statusTypeName+'</button>';
                    }
