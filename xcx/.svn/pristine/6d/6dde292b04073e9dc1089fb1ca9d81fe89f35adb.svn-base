...
class ArticleAjax extends Common{
    ...
    {
        //方式一:九房网新闻获取
        $figure=$this->db2->Name('news')->select('id,title,wap_pic,addtime')->where_equalTo('flag','t,i')->orderBy('addtime','desc')->where_equalTo('belong',15)->orderBy('orders')->limit(0,5)->execute();
        ...
        $data['column'][0]['id']=15;//2
        $data['column'][0]['title']='九房原创';
        $data['column'][1]['id']=1;
        $data['column'][1]['title']='本地资讯';
        //$data['column'][2]['id']=10;
...
//        echo json_encode(['success'=>true,'data'=>$data],JSON_UNESCAPED_UNICODE);
    ...
    public function getArticleData($cid,$curr=1,$limit=10){
        ...
        if(empty($cid)){
            $tmpData=$this->db->Name('xcx_article_article')->select($field)->where_equalTo('status',1)->where_equalTo('is_hot',1)->page($curr,$limit)->orderBy('create_time','desc')->execute();
        }else{
            $tmpData=$this->db->Name('xcx_article_article')->select($field)->where_equalTo('status',1)->where_equalTo('cid',$cid)->page($curr,$limit)->orderBy('create_time','desc')->execute();
        }
        if(!empty($tmpData)){
            //获取后台发布者信息
            $aids=[];$aDict=[];
...
//                    $value['comments_num']=$this->getCommentsNum($value['id']);
    ...
    //获取头条文章数据
    public function getHeadlinesArticle(){
        $cid=0;
        $page=Context::Post('page');
        $data=$this->get9hArticleData($cid,$page);
        if(!empty($data)){
            echo json_encode(['success'=>true,'data'=>$data],JSON_UNESCAPED_UNICODE);
        }else{
            echo json_encode(['success'=>false]);
    ...
    public function getArticleDetail(){
        ...
        //获取文章详情
        $articleInfo=$this->db2->Name('news')->select('title,content,addtime,click,area_id')->where_equalTo('id',$id)->firstRow();
    ...
    /**********************************************获取文章详情数据(作废)***********************************************************/
    //获取文章详情所需数据
    public function getArticleDetailDel(){
        $id=Context::Post('id');
        $articleInfo=$this->db->Name('xcx_article_article')->select()->where_equalTo('id',$id)->where_equalTo('status',1)->firstRow();
        if(empty($articleInfo)){
            $articleInfo=[];
        }else{
        ...
        $data['articleInfo']=$articleInfo;
        //获取该文章所对应的2条评论
  /*      $commentsData=$this->db->Name('xcx_article_comments')->select()->page(1,2)->where_equalTo('aid',$id)->where_equalTo('status',1)->orderBy('create_time','desc')->execute();
        if(empty($commentsData)){
            $commentsData=[];
        }else{
            $uids=[];$uDict=[];     //小程序
    ...
    public function setPraise(){
        $cid=Context::Post('cid');
        $uid=$this->uid();
        if(empty($uid)){echo json_encode(['success'=>false]);exit;}
        $status=empty(Context::Post('status'))?'0':'1';
        $praiseRow=$this->db->Name('xcx_article_comments_praise')->select()->where_equalTo('cid',$cid)->where_equalTo('uid',$uid)->where_equalTo('user_type','1')->firstRow();
        if(empty($praiseRow)){  //添加操作
            $res=$this->db->Name('xcx_article_comments_praise')->insert(['cid'=>$cid,'uid'=>$uid,'user_type'=>'1','status'=>$status,'create_time'=>time(),'update_time'=>time()])->execute();
        }else{  //修改服务
            if((empty($praiseRow['status']) && $status=='1') || (!empty($praiseRow['status']) && $status=='0')){
                $res=$this->db->Name('xcx_article_comments_praise')->update(['status'=>$status,'update_time'=>time()])->where_equalTo('cid',$cid)->where_equalTo('uid',$uid)->where_equalTo('user_type','1')->execute();
            }
        }
        if($res){
            if(empty($status)){ //减少
    ...
    public function addComments(){
        ...
        $rs_content = $this->contentWxCheck($data['content']);
        if($rs_content['errcode']=='87014'){
            return $this->error('抱歉，您含有敏感信息不可提交');
        }
...
//            echo json_encode(['success'=>false]);
    ...
    public function addDetailComments(){
        ...
        if(!empty($parentData['parent_id']) && $parentData['uid']==$data['uid'] && $parentData['user_type']=='1'){
...
//            return $this->error("未登录");
        ...
        if(!empty($commentsData)){
            ...
            $replyRes = $this->db->Name('xcx_article_comments_praise')->select()->where_in('cid', $commentsIds)->where_equalTo('uid',$uid)->where_equalTo('user_type','1')->where_equalTo('status','1')->execute();
            $replyResCids = array_column($replyRes, 'cid');// 获取当前评论里，自己有点赞的评论
            foreach($commentsData as &$comval){
//                $is_reply=$this->db->Name('xcx_article_comments_praise')->select()->where_equalTo('cid',$comval['id'])->where_equalTo('uid',$uid)->where_equalTo('user_type','1')->where_equalTo('status','1')->firstRow();
...
//            return $this->error("未登录");
    ...
    public function addViewsNumber(){
        ...
        $sql="UPDATE ".Table_Pre."xcx_article_article SET `read_num` = `read_num`+1 WHERE `id` = :id";
        $arr=[":id"=>$id];
        DataBase::Update($sql,$arr);
        echo json_encode(['success'=>true]);
    }
